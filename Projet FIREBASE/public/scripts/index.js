    ...
    if (user) {
        ...
        chartRef.on('value', snapshot => {
            ...
            dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot => {
                ...
                var courant1 = jsonData.courant1;
                var courant2 = jsonData.courant2;
                var timestamp = jsonData.timestamp;
                // Plot the values on the charts
                plotValues(chartT, timestamp, temperature);
                plotValues(chartH, timestamp, courant1);
                plotValues(chartP, timestamp, courant2);
            });
        });
        ...
        dbRef.orderByKey().limitToLast(1).on('child_added', snapshot => {
            ...
            humElement.innerHTML = courant1;
            presElement.innerHTML = courant2;
        ...
        dbRef.orderByKey().limitToLast(1).on('child_added', snapshot => {
            ...
            var gaugeH = createCourant1Gauge();
            gaugeT.draw();
            gaugeH.draw();
            gaugeT.value = temperature;
            gaugeH.value = courant1;
            updateElement.innerHTML = epochToDateTime(timestamp);
        });
        ...
        function createTable() {
            ...
            dbRef.orderByKey().limitToLast(100).on('child_added', function(snapshot) {
                if (snapshot.exists()) {
                    ...
                    content += '<td>' + courant1 + '</td>';
                    content += '<td>' + courant2 + '</td>';
                    content += '</tr>';
                ...
                if (snapshot.exists()) {
                    ...
                    reversedList.forEach(element => {
                        if (firstTime) { // ignore first reading (it's already on the table from the previous query)
                            ...
                            var courant1 = element.courant1;
                            var courant2 = element.courant2;
